...
```
PATCH /workflows/{id}
Method note for this host/version (n8n 2.12.2 observed): activation is handled via dedicated endpoints:
- `POST /api/v1/workflows/{id}/activate`
- `POST /api/v1/workflows/{id}/deactivate`

Older `PATCH /workflows/{id}` examples with `{"active": true|false}` are stale here.
```
